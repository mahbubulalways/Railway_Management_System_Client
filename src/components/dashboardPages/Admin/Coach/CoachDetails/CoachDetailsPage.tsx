...
import { useGetSingleCoachQuery } from "@/redux/features/coach.features";
import { ICoach, ISeat } from "@/interface/coach";

const CoachDetailsPage = ({ id }: { id: string }) => {
  const { data, isLoading, isError, error } = useGetSingleCoachQuery(id, {
    refetchOnMountOrArgChange: true,
  });
  console.log(error);

  const coachs = data?.data as ICoach | undefined;

  const rows = useMemo<ISeat[][]>(() => {
    if (!coachs?.seats) return [];

    const grouped = coachs.seats.reduce<Record<number, ISeat[]>>(
      (acc, seat) => {
        if (!acc[seat.row]) {
          acc[seat.row] = [];
        }
  ...
  const getSeatColor = (type: ISeat["seatType"]): string => {
    switch (type) {
      case "WINDOW":
        return "bg-green-500";
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-semibold text-gray-100">
              Coach: {coachs.coachNumber}
            </h1>
            <h1 className="text-3xl font-semibold text-gray-100">
              Code: {coachs.coachCode}
            </h1>
        ...
        <div className="mt-8 grid grid-cols-1 gap-4 md:grid-cols-3 lg:grid-cols-4">
          <div className="rounded-xl  bg-gray-50 p-4">
            ...
            <p className="mt-2 text-lg font-semibold text-gray-900">
              {coachs.coachModel.type.replaceAll("_", " ")}
            </p>
          </div>
          ...
          <div className="rounded-xl  bg-gray-50 p-4">
            ...
            <p className="mt-2 text-gray-700">
              {coachs.coachModel.name || "No description available"}
            </p>
          </div>
